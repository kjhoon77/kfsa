...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={8}>
                    ...
                    <DatePicker format="yyyy/MM/dd" slotProps={{ textField: { size: "small", fullWidth: true, sx: { "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } } } }} />
                 </Stack><Box sx={{ display: hook.isVisible_btnToExcel ? undefined : 'none' }}><Button variant="contained" color="success" startIcon={<Save />} onClick={hook.btnToExcel_OnClick} sx={{ width: '85px', height: '24px' }}>엑셀로 저장</Button></Box></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdList ? undefined : 'none' }}><Paper sx={{ width: '484px', height: '587px', width: '100%', height: 'auto', minHeight: '587px' }}><DataGridWrapper rows={hook.ds_ioList} columns={columns_gdList} /></Paper></Box></Stack>
                ...
                <Grid item xs={12} md={4}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "46px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_btnToExcel2 ? undefined : 'none' }}><Button variant="contained" color="success" startIcon={<Save />} onClick={hook.btnToExcel2_OnClick} sx={{ width: '85px', height: '24px' }}>엑셀로 저장</Button></Box></Stack>
