...
import { useFrmtraining0147PTrainingEvaluationCancel } from './useFrmtraining0147PTrainingEvaluationCancel';
import * as Frmtraining0147PTrainingEvaluationCancelData from './Frmtraining0147PTrainingEvaluationCancelData';

export const Frmtraining0147PTrainingEvaluationCancel = () => {
    const hook = useFrmtraining0147PTrainingEvaluationCancel();
    return (
        <PageContainer>

            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">실무능력평가 합격취소팝업</Typography>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '273px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0147PTrainingEvaluationCancelData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
export default Frmtraining0147PTrainingEvaluationCancel;
