...
import { useFrmtraining0220MTrainingOrderChangeList } from './useFrmtraining0220MTrainingOrderChangeList';
import * as Frmtraining0220MTrainingOrderChangeListData from './Frmtraining0220MTrainingOrderChangeListData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0220MTrainingOrderChangeList = () => {
    const hook = useFrmtraining0220MTrainingOrderChangeList();
    const columns_gdTrainingJubsu = [
        { field: 'TCCOURSENM', headerName: '직능', width: 120 },
        { field: 'JJIBUNICK', headerName: '변경전 지부', width: 80 },
        { field: 'JUBSUNO', headerName: '변경전 접수번호', width: 105 },
        { field: 'AFTERJIBUNICK', headerName: '변경후 지부', width: 80 },
        { field: 'AFTERJUBSUNO', headerName: '변경후 접수번호', width: 105 },
        { field: 'TJPERSONNM', headerName: '성명', width: 78 },
        { field: 'BIRTHDAY', headerName: '생년월일', width: 95 },
        { field: 'TOHPROCDATE', headerName: '회차변경일', width: 90 },
        { field: 'TOHJUBSUGUBUN', headerName: '처리방법', width: 70 },
        { field: 'PPERSONNM', headerName: '처리자', width: 78 },
    ];
    return (
        <PageContainer>

            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">강습회차변경대장</Typography>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '392px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0220MTrainingOrderChangeListData.ds_ds_oSort || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdTrainingJubsu ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '490px', width: '100%', height: 'auto', minHeight: '490px' }}><DataGridWrapper rows={hook.ds_oTrainingJubsu} columns={columns_gdTrainingJubsu} /></Paper></Box></Stack>
...
export default Frmtraining0220MTrainingOrderChangeList;
