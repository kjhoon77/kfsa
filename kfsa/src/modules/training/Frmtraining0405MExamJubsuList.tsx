...
import { useFrmtraining0405MExamJubsuList } from './useFrmtraining0405MExamJubsuList';
import * as Frmtraining0405MExamJubsuListData from './Frmtraining0405MExamJubsuListData';
...
export const Frmtraining0405MExamJubsuList = () => {
    ...
    const columns_gdExamJubsu = [
        ...
        { field: 'EJLASTSUNAPGUBUNNM', headerName: '환불', width: 0 },
        { field: 'EJLASTSUNAPGUBUN', headerName: '환불', width: 80 },
        { field: 'EJPASSYN', headerName: '합격', width: 80 },
        ...
        { field: 'JUBSUGUBUN', headerName: '응시자격', width: 123 },
        { field: 'JUBSUGUBUN_GA', headerName: '가접수여부', width: 119 },
        { field: 'EJZZIPCD', headerName: '우편번호', width: 70 },
        { field: 'TPHPTEL', headerName: '휴대폰', width: 100 },
        { field: 'TPEMAILID', headerName: '이메일', width: 150 },
...
<Button variant="contained" startIcon={<Print />} onClick={hook.lfn_PrintAll}>응시원서일괄출력</Button>
<Button variant="contained" startIcon={<Print />} onClick={hook.lfn_PrintAll}>응시원서일괄출력(개인정보미출력)</Button>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0405MExamJubsuListData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0405MExamJubsuListData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '269px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmtraining0405MExamJubsuListData.ds_ds_oSearchGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '369px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmtraining0405MExamJubsuListData.ds_ds_oRepayYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '369px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>합격여부</Typography>
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmtraining0405MExamJubsuListData.ds_ds_oPassYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "3px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '633px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmtraining0405MExamJubsuListData.ds_ds_ioJubsuGubun2 || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdExamJubsu ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '386px', width: '100%', height: 'auto', minHeight: '386px' }}><DataGridWrapper rows={hook.ds_oExamJubsu} columns={columns_gdExamJubsu} /></Paper></Box></Stack>
...
export default Frmtraining0405MExamJubsuList;
