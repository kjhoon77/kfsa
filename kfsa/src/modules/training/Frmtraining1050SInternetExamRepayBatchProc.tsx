...
import { useFrmtraining1050SInternetExamRepayBatchProc } from './useFrmtraining1050SInternetExamRepayBatchProc';
import * as Frmtraining1050SInternetExamRepayBatchProcData from './Frmtraining1050SInternetExamRepayBatchProcData';
import { FrmCOM3010SPOS } from '../COM/FrmCOM3010SPOS';

export const Frmtraining1050SInternetExamRepayBatchProc = () => {
    const hook = useFrmtraining1050SInternetExamRepayBatchProc();
    ...
    const columns_grdExport = [
        ...
        { field: 'PMAMT', headerName: '입금액', width: 109 },
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining1050SInternetExamRepayBatchProcData.ds_ds_oIProcStatus || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    ...
                    <DatePicker format="yyyy/MM/dd" slotProps={{ textField: { size: "small", fullWidth: true, sx: { "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } } } }} />
                 </Stack><Box sx={{ display: hook.isVisible_btnPatchRepay ? undefined : 'none' }}><Button variant="contained"  onClick={hook.lfn_Check} sx={{ width: '209px', height: '24px' }}>송금후 일괄환불처리 (엑셀로딩)</Button></Box></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_grdExport ? undefined : 'none' }}><Paper sx={{ width: '20px', height: '62px', width: '100%', height: 'auto', minHeight: '62px' }}><DataGridWrapper rows={hook.ds_ioRepayBundleCopy} columns={columns_grdExport} /></Paper></Box><Box sx={{ display: hook.isVisible_fdImport ? undefined : 'none' }}><Box sx={{ width: '24px', height: '26px', border: '1px dashed grey' }}>Unknown: FileDialog</Box></Box><Box sx={{ display: hook.isVisible_divPos ? undefined : 'none' }}><Paper sx={{ p: 2, width: '128px', height: '66px', width: '100%', height: 'auto', minHeight: '66px', overflow: 'auto' }}><Typography variant="subtitle2" sx={{ mb: 1, fontWeight: "bold" }}>Div0</Typography><Stack direction="column" spacing={0}><FrmCOM3010SPOS /></Stack></Paper></Box></Stack>
...
export default Frmtraining1050SInternetExamRepayBatchProc;
