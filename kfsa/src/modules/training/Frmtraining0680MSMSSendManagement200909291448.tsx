...
import { useFrmtraining0680MSMSSendManagement200909291448 } from './useFrmtraining0680MSMSSendManagement200909291448';
import * as Frmtraining0680MSMSSendManagement200909291448Data from './Frmtraining0680MSMSSendManagement200909291448Data';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0680MSMSSendManagement200909291448 = () => {
    const hook = useFrmtraining0680MSMSSendManagement200909291448();
    const columns_gdSms = [
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0680MSMSSendManagement200909291448Data.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0680MSMSSendManagement200909291448Data.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '392px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmtraining0680MSMSSendManagement200909291448Data.ds_ds_oRepayYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ ml: "8px", minWidth: "492px" }}><Box sx={{ display: hook.isVisible_lbExamPassYn ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '88px', height: '22px' }}><Typography variant="body2">합격여부</Typography></Box></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '492px', height: '22px', display: 'flex', alignItems: 'center', ml: '0px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmtraining0680MSMSSendManagement200909291448Data.ds_ds_oExamPassYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack><Box sx={{ display: hook.isVisible_radTrainingPassYn ? undefined : 'none' }}><FormControl component="fieldset" sx={{ width: '300px', height: '22px' }}><RadioGroup row >{ (Frmtraining0680MSMSSendManagement200909291448Data.ds_ds_oTrainingPassYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl></Box></Box></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "7px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_btnMutilSort ? undefined : 'none' }}><Button variant="outlined" onClick={hook.btnMutilSort_OnClick} size="small" sx={{ minWidth: 26, p: 0.2, width: '74px', height: '22px' }}><Sort fontSize="small" /></Button></Box><Box sx={{ display: hook.isVisible_imgSmsTraining ? undefined : 'none' }}><Box sx={{ width: '158px', height: '369px', border: "1px solid #ddd", display: "flex", alignItems: "center", justifyContent: "center" }}>Image: imgSmsTraining</Box></Box></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "35px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_btnSend ? undefined : 'none' }}><Button variant="contained"  onClick={hook.lfn_Send} sx={{ width: '85px', height: '24px' }}>보내기</Button></Box></Stack>
...
export default Frmtraining0680MSMSSendManagement200909291448;
