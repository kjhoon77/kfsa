...
import { useFrmtraining0600MExamSupervisorManagement } from './useFrmtraining0600MExamSupervisorManagement';
import * as Frmtraining0600MExamSupervisorManagementData from './Frmtraining0600MExamSupervisorManagementData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0600MExamSupervisorManagement = () => {
    const hook = useFrmtraining0600MExamSupervisorManagement();
    const columns_gdExamSupervisor = [
        ...
        { field: 'SIBDAY', headerName: '생년월일', width: 76 },
        { field: 'SISEXCD', headerName: '성별', width: 37 },
        { field: 'SIPERSONYN', headerName: '감독관구분', width: 76 },
        { field: 'SITEL', headerName: '전화번호', width: 100 },
        { field: 'SIHPTEL', headerName: '핸드폰번호', width: 100 },
        { field: 'SISTARTDATE', headerName: '위촉시작일', width: 80 },
        { field: 'SIENDDATE', headerName: '위촉종료일', width: 80 },
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0600MExamSupervisorManagementData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '198px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0600MExamSupervisorManagementData.ds_ds_oSex || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0600MExamSupervisorManagementData.ds_ds_oPersonYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "3px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '198px', height: '22px', display: 'flex', alignItems: 'center', ml: '9px' }}>
                    ...
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack><Box sx={{ display: hook.isVisible_btnSendSms ? undefined : 'none' }}><Button variant="contained"   onClick={hook.btnSendSms_OnClick} sx={{ width: '66px', height: '24px', whiteSpace: "nowrap" }}>문자발송</Button></Box></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "3px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '190px', height: '22px', display: 'flex', alignItems: 'center', ml: '9px' }}>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdExamSupervisor ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '380px', width: '100%', height: 'auto', minHeight: '380px' }}><DataGridWrapper rows={hook.ds_ioExamSupervisor} columns={columns_gdExamSupervisor} /></Paper></Box></Stack>
...
export default Frmtraining0600MExamSupervisorManagement;
