...
import { useFrmtraining0421MExamAbsentManagement } from './useFrmtraining0421MExamAbsentManagement';
import * as Frmtraining0421MExamAbsentManagementData from './Frmtraining0421MExamAbsentManagementData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0421MExamAbsentManagement = () => {
    const hook = useFrmtraining0421MExamAbsentManagement();
    const columns_gdExamAbsent = [
        { field: 'SEL', headerName: '결강', width: 40 },
        { field: 'CHASUGUBUN', headerName: '접수구분', width: 61 },
        ...
        { field: 'EJADDR2', headerName: '주소2', width: 143 },
    ];
    return (
        <PageContainer>
...
<Button variant="contained"  onClick={hook.lfn_Save}>결시처리(F4)</Button>
<Button variant="contained" startIcon={<Refresh />} onClick={hook.btnCancelAbsent_OnClick}>결시취소</Button>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "3px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '336px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>결시구분</Typography>
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0421MExamAbsentManagementData.ds_ds_oAbsentChasuGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "8px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '180px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>결시자수</Typography>
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdExamAbsent ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '490px', width: '100%', height: 'auto', minHeight: '490px' }}><DataGridWrapper rows={hook.ds_ioExamAbsent} columns={columns_gdExamAbsent} /></Paper></Box></Stack>
...
export default Frmtraining0421MExamAbsentManagement;
