...
import { useFrmtraining0395MExamScheduleList } from './useFrmtraining0395MExamScheduleList';
import * as Frmtraining0395MExamScheduleListData from './Frmtraining0395MExamScheduleListData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0395MExamScheduleList = () => {
    const hook = useFrmtraining0395MExamScheduleList();
    const columns_gdExamOrder = [
        { field: 'EOMGJIBUCD', headerName: '지부', width: 58 },
        { field: 'TCCOURSECD', headerName: '직능', width: 99 },
        { field: 'EOYEAR', headerName: '년도', width: 48 },
        { field: 'EOEXAMORDER', headerName: '회차', width: 39 },
        { field: 'EOEXAMORDERGUBUN', headerName: '회차구분', width: 109 },
        { field: 'EOCHASUGUBUN', headerName: '특급 차수', width: 32 },
        { field: 'EOEXAMDATE', headerName: '시험일자', width: 91 },
        { field: 'EOSTARTTIME', headerName: '시험시간', width: 60 },
        { field: 'EOSEATCNT', headerName: '시험 정원', width: 40 },
        { field: 'EOJUBSUMAXCNT', headerName: '접수 완료', width: 40 },
    ...
    return (
        <PageContainer>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">시험일정현황</Typography>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "62px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_radSearchGubun ? undefined : 'none' }}><FormControl component="fieldset" sx={{ width: '76px', height: '42px', width: "max-content", whiteSpace: "nowrap", pr: 2 }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0395MExamScheduleListData.ds_ds_oSearchGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '120px', height: '22px', display: 'flex', alignItems: 'center', ml: '0px' }}>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdExamOrder ? undefined : 'none' }}><Paper sx={{ width: '892px', height: '490px', width: '100%', height: 'auto', minHeight: '490px' }}><DataGridWrapper rows={hook.ds_oExamOrder} columns={columns_gdExamOrder} /></Paper></Box></Stack>
...
export default Frmtraining0395MExamScheduleList;
