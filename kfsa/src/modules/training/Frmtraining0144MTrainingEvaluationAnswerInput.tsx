...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '184px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0144MTrainingEvaluationAnswerInputData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "16px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '320px', height: '24px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 80, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>평가구분</Typography>
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0144MTrainingEvaluationAnswerInputData.ds_ds_oEvalGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack><Box sx={{ display: hook.isVisible_btnSave ? undefined : 'none' }}><Button variant="contained" color="success"  onClick={hook.lfn_Save} sx={{ width: '145px', height: '24px', whiteSpace: "nowrap" }}>① 답안엑셀등록</Button></Box><Box sx={{ display: hook.isVisible_btnMarking ? undefined : 'none' }}><Button variant="contained"   onClick={hook.btnMarking_OnClick} sx={{ width: '145px', height: '24px', whiteSpace: "nowrap" }}>② 채점</Button></Box><Box sx={{ display: hook.isVisible_progMarkingCheck ? undefined : 'none' }}><Box sx={{ width: '180px', height: '22px', border: '1px dashed grey' }}>Unknown: Progressbar</Box></Box></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdEvalList ? undefined : 'none' }}><Paper sx={{ width: '1269px', height: '506px', width: '100%', height: 'auto', minHeight: '506px' }}><DataGridWrapper rows={hook.ds_ioEvaluationAnswerInput} columns={columns_gdEvalList} /></Paper></Box></Stack>
