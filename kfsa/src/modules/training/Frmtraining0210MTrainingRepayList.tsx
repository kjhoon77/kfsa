...
import { useFrmtraining0210MTrainingRepayList } from './useFrmtraining0210MTrainingRepayList';
import * as Frmtraining0210MTrainingRepayListData from './Frmtraining0210MTrainingRepayListData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0210MTrainingRepayList = () => {
    const hook = useFrmtraining0210MTrainingRepayList();
    const columns_gdTrainingJubsu = [
        { field: 'GTDEPTNM', headerName: '지부', width: 60 },
        { field: 'TOTCCOURSECD', headerName: '직능', width: 98 },
        ...
        { field: 'GUBUN', headerName: '구분', width: 60 },
        { field: 'TSPROCAMOUNT', headerName: '환불금액', width: 95 },
        { field: 'TSPROCDATE', headerName: '환불일', width: 90 },
        { field: 'PNM', headerName: '처리자', width: 60 },
        { field: 'SUNAPGUBUN', headerName: '접수방법', width: 90 },
    ];
    return (
        <PageContainer>

            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">강습환불자대장</Typography>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>환불일자</Typography>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '392px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0210MTrainingRepayListData.ds_ds_oSort || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdTrainingJubsu ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '490px', width: '100%', height: 'auto', minHeight: '490px' }}><DataGridWrapper rows={hook.ds_oTrainingRepay} columns={columns_gdTrainingJubsu} /></Paper></Box></Stack>
...
export default Frmtraining0210MTrainingRepayList;
