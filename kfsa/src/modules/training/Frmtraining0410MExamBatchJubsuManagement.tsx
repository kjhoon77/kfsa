...
import { useFrmtraining0410MExamBatchJubsuManagement } from './useFrmtraining0410MExamBatchJubsuManagement';
import * as Frmtraining0410MExamBatchJubsuManagementData from './Frmtraining0410MExamBatchJubsuManagementData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0410MExamBatchJubsuManagement = () => {
    const hook = useFrmtraining0410MExamBatchJubsuManagement();
    const columns_gdTrainingExam = [
        { field: 'SEL', headerName: '', width: 29 },
        { field: 'TOTCCOURSECD', headerName: '수료한직능명', width: 148 },
        { field: 'TOHJUBSUNO', headerName: '교육접수번호', width: 92 },
        { field: 'TPNM', headerName: '성명', width: 73 },
        { field: 'TJBIRTHDAY_SANG', headerName: '생년월일', width: 101 },
    ...
    return (
        <PageContainer>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">시험일괄접수</Typography>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "61px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_lbPassOrder ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '150px', height: '22px' }}><Typography variant="body2">수료한 회차</Typography></Box></Box><Box sx={{ display: hook.isVisible_lbPassCourse ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '110px', height: '22px' }}><Typography variant="body2">직능</Typography></Box></Box><Box sx={{ display: hook.isVisible_cbxCourse ? undefined : 'none' }}><FormControl size="small" fullWidth sx={{ width: '210px', height: '22px', "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" }, bgcolor: "#fff" }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0410MExamBatchJubsuManagementData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '161px', height: '22px', display: 'flex', alignItems: 'center', ml: '16px' }}>
                    <Typography variant="body2" sx={{ minWidth: 110, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>회차</Typography>
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_lbJubsuOrder ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '150px', height: '22px' }}><Typography variant="body2">시험 접수할 회차</Typography></Box></Box><Box sx={{ display: hook.isVisible_lbJubsuCourse ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '110px', height: '22px' }}><Typography variant="body2">직능</Typography></Box></Box><Box sx={{ ml: "21px", minWidth: "210px" }}><Box sx={{ display: hook.isVisible_cbxJubsuCourse1 ? undefined : 'none' }}><FormControl size="small" fullWidth sx={{ width: '210px', height: '22px', "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" }, bgcolor: "#fff" }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0410MExamBatchJubsuManagementData.ds_ds_oJibu_FilterBak || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl></Box><Box sx={{ display: hook.isVisible_cbxJubsuCourse ? undefined : 'none' }}><FormControl size="small" fullWidth sx={{ width: '210px', height: '22px', "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" }, bgcolor: "#fff" }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0410MExamBatchJubsuManagementData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl></Box></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '161px', height: '22px', display: 'flex', alignItems: 'center', ml: '16px' }}>
                    <Typography variant="body2" sx={{ minWidth: 110, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>회차</Typography>
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "10px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '170px', height: '22px', display: 'flex', alignItems: 'center', ml: '4px' }}>
                    ...
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack><Box sx={{ display: hook.isVisible_Static0 ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '510px', height: '28px' }}><Typography variant="body2">※특급,1급,2급 소방안전관리자의 경우
   강습교육 접수시 등록한 개인정보동의서가 시험용 개인정보동의서로 자동등록 됩니다.</Typography></Box></Box></Stack>
...
export default Frmtraining0410MExamBatchJubsuManagement;
