...
import { useFrmtraining0407MExamJubsuGubunList } from './useFrmtraining0407MExamJubsuGubunList';
import * as Frmtraining0407MExamJubsuGubunListData from './Frmtraining0407MExamJubsuGubunListData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmtraining0407MExamJubsuGubunList = () => {
    const hook = useFrmtraining0407MExamJubsuGubunList();
    const columns_gdExamJubsu = [
        { field: 'EOMGGTMGNO', headerName: '지부', width: 92 },
        { field: 'EOYEAR', headerName: '년도', width: 61 },
        { field: 'EOTCCOURSECD', headerName: '직능', width: 141 },
        { field: 'EJPASSYN', headerName: '합격여부', width: 77 },
        { field: 'TOTALCNT', headerName: '계', width: 100 },
        { field: 'JUBGUBUN20', headerName: '강습수료', width: 100 },
        { field: 'JUBGUBUN00', headerName: '경력', width: 100 },
        { field: 'JUBGUBUN01', headerName: '학력', width: 100 },
        { field: 'JUBGUBUN02', headerName: '학경력', width: 100 },
        { field: 'JUBGUBUN05', headerName: '자격·경력', width: 100 },
        { field: 'JUBGUBUN06', headerName: '자격', width: 100 },
    ];
    return (
        <PageContainer>

            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">시험응시자격별 현황</Typography>
...
<Button variant="contained" startIcon={<Print />} onClick={hook.lfn_Print}>현황 출력</Button>
<Button variant="contained" startIcon={<Print />} onClick={hook.lfn_PrintAll}>리스트출력</Button>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0407MExamJubsuGubunListData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '464px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0407MExamJubsuGubunListData.ds_ds_oJubsuGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '464px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>합격여부</Typography>
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmtraining0407MExamJubsuGubunListData.ds_ds_oPassYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdExamJubsu ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '418px', width: '100%', height: 'auto', minHeight: '418px' }}><DataGridWrapper rows={hook.ds_oExamJubsuTotal} columns={columns_gdExamJubsu} /></Paper></Box></Stack>
...
export default Frmtraining0407MExamJubsuGubunList;
