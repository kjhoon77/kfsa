...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '214px', height: '22px', display: 'flex', alignItems: 'center', ml: '4px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (FormData.ds_ds_ioSelectGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_Static60 ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '514px', height: '24px' }}><Typography variant="body2">※ 고객 정보를 더블클릭 시 우측 국민안전처 자료 자동조회</Typography></Box></Box></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdMFManager ? undefined : 'none' }}><Paper sx={{ width: '513px', height: '167px', width: '100%', height: 'auto', minHeight: '167px' }}><DataGridWrapper rows={hook.ds_ioFireManagerInfo2} columns={columns_gdMFManager_1} /></Paper></Box></Stack>
