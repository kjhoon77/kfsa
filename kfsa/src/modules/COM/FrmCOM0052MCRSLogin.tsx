...
import { useFrmCOM0052MCRSLogin } from './useFrmCOM0052MCRSLogin';
import * as FrmCOM0052MCRSLoginData from './FrmCOM0052MCRSLoginData';

export const FrmCOM0052MCRSLogin = () => {
    const hook = useFrmCOM0052MCRSLogin();
    return (
        <PageContainer>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '278px', height: '22px', display: 'flex', alignItems: 'center', ml: '263px' }}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (FrmCOM0052MCRSLoginData.ds_gds_oSameUserInfo || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
export default FrmCOM0052MCRSLogin;
