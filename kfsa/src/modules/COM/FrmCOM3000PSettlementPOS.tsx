...
import { useFrmCOM3000PSettlementPOS } from './useFrmCOM3000PSettlementPOS';
import * as FrmCOM3000PSettlementPOSData from './FrmCOM3000PSettlementPOSData';
import { FrmCOM3010SPOS } from '../COM/FrmCOM3010SPOS';
import { FrmCOM3011SPOS } from '../COM/FrmCOM3011SPOS';
import { FrmCOM3051SSettlementPOSRepay } from '../COM/FrmCOM3051SSettlementPOSRepay';
import { FrmCOM3100SSettlementPOSSunap } from '../COM/FrmCOM3100SSettlementPOSSunap';

export const FrmCOM3000PSettlementPOS = () => {
    const hook = useFrmCOM3000PSettlementPOS();
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '223px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (FrmCOM3000PSettlementPOSData.ds_ds_oTrainingRepayFee || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack><Box sx={{ ml: "1px", minWidth: "560px" }}><Box sx={{ display: hook.isVisible_edProcAmount ? undefined : 'none' }}><TextField size="small" fullWidth  sx={{ width: '560px', height: '22px', "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" }, bgcolor: "#fff" }} /></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '223px', height: '22px', display: 'flex', alignItems: 'center', ml: '0px' }}>
                    <Typography variant="body2" sx={{ minWidth: 100, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>결강일차</Typography>
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (FrmCOM3000PSettlementPOSData.ds_ds_oTrainingAbsentDay || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Box></Stack>
...
export default FrmCOM3000PSettlementPOS;
