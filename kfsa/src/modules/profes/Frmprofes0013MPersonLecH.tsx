...
import { useFrmprofes0013MPersonLecH } from './useFrmprofes0013MPersonLecH';
import * as Frmprofes0013MPersonLecHData from './Frmprofes0013MPersonLecHData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmprofes0013MPersonLecH = () => {
    const hook = useFrmprofes0013MPersonLecH();
    ...
    const columns_Grid0 = [
        ...
        { field: 'CNT1', headerName: '실무 건수', width: 80 },
        { field: 'TIME1', headerName: '시간', width: 80 },
        { field: 'CNT2', headerName: '강습 건수', width: 80 },
        { field: 'TIME2', headerName: '시간', width: 80 },
        { field: 'CNT3', headerName: '전문교육 건수', width: 80 },
        { field: 'TIME3', headerName: '시간', width: 80 },
    ];
    const columns_gdList = [
        { field: 'id_seq', headerName: '순번 총계', width: 41, renderCell: (params) => params.api.getAllRowIds().indexOf(params.id) + 1, sortable: false },
        { field: 'LDPFNAME', headerName: '교수', width: 130 },
        { field: 'GTDEPTNM', headerName: '지부', width: 70 },
        { field: 'GUBUN', headerName: '구분', width: 50 },
    ...
    return (
        <PageContainer>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">개인별 교육현황</Typography>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "5px", py: 0.5, width: "100%" }}><Box sx={{ ml: "3px", minWidth: "780px" }}><Box sx={{ display: hook.isVisible_Grid0 ? undefined : 'none' }}><Paper sx={{ width: '780px', height: '578px', width: '100%', height: 'auto', minHeight: '578px' }}><MultiDataGridWrapper rows={hook.ds_ProfesList} columns={columns_Grid0} rowHeight={20} headerHeight={40} hideFooter={true} /></Paper></Box><Box sx={{ display: hook.isVisible_gdList ? undefined : 'none' }}><Paper sx={{ width: '780px', height: '578px', width: '100%', height: 'auto', minHeight: '578px' }}><MultiDataGridWrapper rows={hook.ds_ProfesList} columns={columns_gdList} rowHeight={20} headerHeight={40} hideFooter={true} /></Paper></Box></Box></Stack>
...
export default Frmprofes0013MPersonLecH;
