...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_rdGubun ? undefined : 'none' }}><FormControl component="fieldset" sx={{ width: '88px', height: '294px' }}><RadioGroup row >{ (Frmcust3030PAsstnLicenseListData.ds_ds_iGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl></Box><Box sx={{ display: hook.isVisible_gdLicense ? undefined : 'none' }}><Paper sx={{ width: '560px', height: '142px', width: '100%', height: 'auto', minHeight: '142px' }}><DataGridWrapper rows={hook.ds_oLicenseList} columns={columns_gdLicense} /></Paper></Box></Stack>
