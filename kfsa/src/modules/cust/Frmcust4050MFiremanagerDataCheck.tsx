...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmcust4050MFiremanagerDataCheckData.ds_ds_ioChoiceYn || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '138px', height: '22px', display: 'flex', alignItems: 'center', ml: '6px' }}>
                    ...
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack><Box sx={{ display: hook.isVisible_btnFireStation ? undefined : 'none' }}><Button variant="contained"  onClick={hook.FireStation_Onclick} sx={{ width: '145px', height: '24px' }}>소방관서 자료 불러오기</Button></Box><Box sx={{ display: hook.isVisible_btnFireStationCheck ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnFireStationCheck_OnClick} sx={{ width: '92px', height: '24px' }}>자료 비교 하기</Button></Box><Box sx={{ display: hook.isVisible_btnResult ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnResult_OnClick} sx={{ width: '92px', height: '24px' }}>결과 보기</Button></Box></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_fdImport ? undefined : 'none' }}><Box sx={{ width: '24px', height: '26px', border: '1px dashed grey' }}>Unknown: FileDialog</Box></Box></Stack>
