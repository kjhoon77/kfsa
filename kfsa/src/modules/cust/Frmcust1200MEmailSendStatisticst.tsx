...
import { useFrmcust1200MEmailSendStatisticst } from './useFrmcust1200MEmailSendStatisticst';
import * as Frmcust1200MEmailSendStatisticstData from './Frmcust1200MEmailSendStatisticstData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmcust1200MEmailSendStatisticst = () => {
    const hook = useFrmcust1200MEmailSendStatisticst();
    const columns_gdEmailStat = [
        ...
        { field: 'ESDATE', headerName: '교육일자', width: 74 },
        { field: 'ESTIME', headerName: '교육시간', width: 107 },
        { field: 'GTDEPTNM', headerName: '지부', width: 60 },
        { field: 'CNO', headerName: '관리번호', width: 64 },
        { field: 'RCNM', headerName: '지역', width: 52 },
        { field: 'ESCOURSENM', headerName: '직능', width: 103 },
        { field: 'CSTATUSGUBUN', headerName: '회원구분', width: 60 },
        { field: 'FMNM', headerName: '수신자', width: 60 },
        { field: 'ESHEMAILADDR', headerName: '이메일', width: 135 },
        { field: 'ESHRECEIVEYN', headerName: '수신확인', width: 60 },
        { field: 'ESHSENDYN', headerName: '전송상태', width: 60 },
        { field: 'ESHSENDDATE', headerName: '발송시간', width: 129 },
        { field: 'PNM', headerName: '발송자', width: 59 },
        { field: 'ECTITLE', headerName: '메일제목', width: 209 },
    ];
    return (
        <PageContainer>

            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">[인터넷]메일발송현황</Typography>
...
<Button variant="contained" startIcon={<Print />} onClick={hook.btnPrintJibu_OnClick}>발송현황(지부별) 출력</Button>
<Button variant="contained" startIcon={<Print />} onClick={hook.btnPrintDate_OnClick}>발송현황(발송일자별) 출력</Button>
<Button variant="contained" startIcon={<Print />} onClick={hook.btnPrintList_OnClick}>발송리스트 출력</Button>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        ...
                        <Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "62px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '173px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>지부</Typography>
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmcust1200MEmailSendStatisticstData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmcust1200MEmailSendStatisticstData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdEmailStat ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '490px', width: '100%', height: 'auto', minHeight: '490px' }}><DataGridWrapper rows={hook.ds_oEmailStat} columns={columns_gdEmailStat} /></Paper></Box></Stack>
...
export default Frmcust1200MEmailSendStatisticst;
