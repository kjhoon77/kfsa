...
export const Frmcust1350MCAGDMData = () => {
    ...
    const columns_gdExamOrder = [
        { field: 'SELA', headerName: '', width: 50 },
        { field: 'col_1', headerName: '순번', width: 50 },
        ...
        { field: 'TRAININGEXPIREDATE', headerName: '', width: 128 },
        { field: 'TJREGDATE', headerName: '', width: 133 },
        { field: 'TPPASSDATE', headerName: '', width: 133 },
    ];
    const columns_Grid1 = [
    ...
    return (
        <PageContainer>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">업무대행감독 DM자료 생성</Typography>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "3px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '299px', height: '22px', display: 'flex', alignItems: 'center', ml: '16px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmcust1350MCAGDMDataData.ds_ds_oCourse || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack><Box sx={{ display: hook.isVisible_cldAPNT_DE_8 ? undefined : 'none' }}><DoubleClickDatePicker sx={{ width: "102px", "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 0px", fontSize: "12px", marginLeft: "-2px" } }} /></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '113px', height: '22px', display: 'flex', alignItems: 'center', ml: '3px' }}>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '299px', height: '22px', display: 'flex', alignItems: 'center', ml: '16px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmcust1350MCAGDMDataData.ds_ds_2 || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack><Box sx={{ display: hook.isVisible_radGUBUN2 ? undefined : 'none' }}><FormControl component="fieldset" sx={{ width: '320px', height: '22px', width: "max-content", whiteSpace: "nowrap", pr: 2 }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmcust1350MCAGDMDataData.ds_ds_3 || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl></Box><Box sx={{ display: hook.isVisible_btnExcelPrint1 ? undefined : 'none' }}><Button variant="contained" color="success"  onClick={hook.btnExcelPrint1_OnClick} sx={{ width: '184px', height: '49px', whiteSpace: "nowrap" }}>출력(EXCEL)</Button></Box></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '299px', height: '22px', display: 'flex', alignItems: 'center', ml: '16px' }}>
                    <Typography variant="body2" sx={{ minWidth: 104, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>강습교육접수자</Typography>
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmcust1350MCAGDMDataData.ds_ds_oTJubsu || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_lbTextBlue ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '524px', height: '55px' }}><Typography variant="body2">※ 유효기간 선택은 수료구분이 [ 수료 ] 일때만 선택이 가능합니다.
 ...
 조회 DATA중 빨간색 글씨는 유효기간 경과를 의미합니다.</Typography></Box></Box><Box sx={{ display: hook.isVisible_btnSms ? undefined : 'none' }}><Button variant="contained"   onClick={hook.btnSms_OnClick} sx={{ width: '184px', height: '49px', whiteSpace: "nowrap" }}>문자발송</Button></Box></Stack>
