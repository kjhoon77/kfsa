...
export const Frmspcledu0655MExamFailReJubsu = () => {
    const hook = useFrmspcledu0655MExamFailReJubsu();
    ...
    const columns_gdTrainingExam = [
        ...
        { field: 'AMT', headerName: '선수납', width: 60 },
        { field: 'TSGUBUNCD', headerName: '환불여부', width: 70 },
        { field: 'REJUBSUNO', headerName: '재접수번호', width: 80 },
        { field: 'EJHSTATUSCD', headerName: '결제여부', width: 80 },
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmspcledu0655MExamFailReJubsuData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ ml: "12px", minWidth: "536px" }}><Box sx={{ display: hook.isVisible_lbSunapMsg ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '508px', height: '22px' }}><Typography variant="body2">※ 결제 유형이 [현금]인 경우에만 자동접수 하시기 바랍니다.</Typography></Box></Box><FormControlLabel control={<Checkbox checked={chk_chkJubsuProcGubun === '1'} onChange={(e) => setChk_chkJubsuProcGubun(e.target.checked ? '1' : '0')} />} label="수납정보포함 접수처리" /></Box><Box sx={{ display: hook.isVisible_btnSearchExamOrder ? undefined : 'none' }}><Button variant="contained"   onClick={hook.btnSearchExamOrder_OnClick} sx={{ width: '92px', height: '24px', whiteSpace: "nowrap" }}>접수가능회차</Button></Box></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '294px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>수납방법</Typography>
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmspcledu0655MExamFailReJubsuData.ds_ds_oSunapGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "6px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '170px', height: '22px', display: 'flex', alignItems: 'center', ml: '4px' }}>
                    ...
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '128px', height: '22px', display: 'flex', alignItems: 'center', ml: '18px' }}>
...
export default Frmspcledu0655MExamFailReJubsu;
