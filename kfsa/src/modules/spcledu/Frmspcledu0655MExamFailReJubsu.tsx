...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '294px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmspcledu0655MExamFailReJubsuData.ds_ds_oSunapGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack><Box sx={{ display: hook.isVisible_btnPrintRctTotal ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnPrintRctTotal_OnClick} sx={{ width: '131px', height: '24px' }}>영수증 일괄 출력</Button></Box></Stack>
