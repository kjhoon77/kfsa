...
import { useFrmspcledu0041PTrainingAbsentCancel } from './useFrmspcledu0041PTrainingAbsentCancel';
import * as Frmspcledu0041PTrainingAbsentCancelData from './Frmspcledu0041PTrainingAbsentCancelData';

export const Frmspcledu0041PTrainingAbsentCancel = () => {
    const hook = useFrmspcledu0041PTrainingAbsentCancel();
    return (
        <PageContainer>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '273px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmspcledu0041PTrainingAbsentCancelData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
export default Frmspcledu0041PTrainingAbsentCancel;
