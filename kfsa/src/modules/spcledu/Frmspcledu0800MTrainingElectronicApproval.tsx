...
import { useFrmspcledu0800MTrainingElectronicApproval } from './useFrmspcledu0800MTrainingElectronicApproval';
import * as Frmspcledu0800MTrainingElectronicApprovalData from './Frmspcledu0800MTrainingElectronicApprovalData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmspcledu0800MTrainingElectronicApproval = () => {
    const hook = useFrmspcledu0800MTrainingElectronicApproval();
    const columns_gdElecAppr = [
        { field: 'ROWNUM', headerName: '순번', width: 60 },
        { field: 'PROCDATE', headerName: '수납일자(기간)', width: 100 },
        { field: 'RE_PC_DEPARTMENT', headerName: '처리부서', width: 60 },
        { field: 'RE_PC_POSITION', headerName: '직위', width: 60 },
        { field: 'RE_APPROVER_NAME', headerName: '결재자', width: 80 },
        { field: 'RECEIVE_DATE', headerName: '결재일자', width: 60 },
        { field: 'RE_APPROVER_KIND', headerName: '결재유형', width: 60 },
        { field: 'RE_APPROVER_STATUS', headerName: '결재상태', width: 60 },
    ];
    return (
        <PageContainer>

            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">강습관련전자결재연동</Typography>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmspcledu0800MTrainingElectronicApprovalData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_lbProcMsg3 ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '444px', height: '22px' }}><Typography variant="body2">3. 인터넷 환불 현황</Typography></Box></Box><Box sx={{ display: hook.isVisible_btnCreateList ? undefined : 'none' }}><Button variant="contained" color="secondary"  onClick={hook.btnCreateList_OnClick} sx={{ width: '314px', height: '24px', whiteSpace: "nowrap" }}>      (1)  세부내역 리스트 생성 (전자결재 첨부용)</Button></Box></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_lbProcMsg6 ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '15px', height: '22px' }}><Typography variant="body2">※</Typography></Box></Box><Box sx={{ display: hook.isVisible_lbProcMsg7 ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '420px', height: '22px' }}><Typography variant="body2">1, 2, 3, 4에 대한 세부내역 리스트를 PDF 파일로 미리 생성해 놓으신 후</Typography></Box></Box><Box sx={{ display: hook.isVisible_btnProcElecApv ? undefined : 'none' }}><Button variant="contained"   onClick={hook.btnProcElecApv_OnClick} sx={{ width: '314px', height: '24px', whiteSpace: "nowrap" }}>      (2) 전자결재 연동 처리 (기안)</Button></Box></Stack>
...
export default Frmspcledu0800MTrainingElectronicApproval;
