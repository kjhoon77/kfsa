...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl component="fieldset"><RadioGroup row >{ (Frmspcledu0280MPassDiplomaPrintData.ds_ds_oSearchGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '180px', height: '22px', display: 'flex', alignItems: 'center', ml: '4px' }}>
                 ...
                 </Stack><Box sx={{ ml: "1px", minWidth: "166px" }}><Box sx={{ display: hook.isVisible_btnPass ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnPass_OnClick} sx={{ width: '92px', height: '24px' }}>수료증미리보기</Button></Box><Box sx={{ display: hook.isVisible_btnPassPrint ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnPassPrint} sx={{ width: '92px', height: '24px' }}>수료증출력</Button></Box></Box><Box sx={{ display: hook.isVisible_btnPassList ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnPassList_OnClick} sx={{ width: '92px', height: '24px' }}>수료대장출력</Button></Box><Box sx={{ display: hook.isVisible_btnAttend ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnAttend_OnClick} sx={{ width: '92px', height: '24px' }}>참가증출력</Button></Box><Box sx={{ display: hook.isVisible_btnMolPass ? undefined : 'none' }}><Button variant="contained"  onClick={hook.btnMolPass_OnClick} sx={{ width: '170px', height: '24px' }}>노동부제출용 수료증출력</Button></Box><Box sx={{ display: hook.isVisible_btnMutilSort ? undefined : 'none' }}><Button variant="outlined" onClick={hook.btnMutilSort_OnClick} size="small" sx={{ minWidth: 26, p: 0.2, width: '74px', height: '22px' }}><Sort fontSize="small" /></Button></Box></Stack>
