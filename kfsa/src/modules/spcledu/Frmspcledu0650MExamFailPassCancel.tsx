...
import { useFrmspcledu0650MExamFailPassCancel } from './useFrmspcledu0650MExamFailPassCancel';
import * as Frmspcledu0650MExamFailPassCancelData from './Frmspcledu0650MExamFailPassCancelData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmspcledu0650MExamFailPassCancel = () => {
    const hook = useFrmspcledu0650MExamFailPassCancel();
    const columns_gdExamFail = [
        ...
        { field: 'EJRESIDENTNO', headerName: '생년월일', width: 96 },
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdExamFail ? undefined : 'none' }}><Paper sx={{ width: '548px', height: '502px', width: '100%', height: 'auto', minHeight: '502px' }}><DataGridWrapper rows={hook.ds_ioExamFailCountList} columns={columns_gdExamFail} /></Paper></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '180px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    <Typography variant="body2" sx={{ minWidth: 88, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>합격취소인원</Typography>
                    <TextField size="small" fullWidth  sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 6px" } }} />
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_gdPassCancel ? undefined : 'none' }}><Paper sx={{ width: '240px', height: '478px', width: '100%', height: 'auto', minHeight: '478px' }}><DataGridWrapper rows={hook.ds_ioExamPassCancelList} columns={columns_gdPassCancel} /></Paper></Box></Stack>
...
export default Frmspcledu0650MExamFailPassCancel;
