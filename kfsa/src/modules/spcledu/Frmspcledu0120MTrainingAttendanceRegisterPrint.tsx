...
import { useFrmspcledu0120MTrainingAttendanceRegisterPrint } from './useFrmspcledu0120MTrainingAttendanceRegisterPrint';
import * as Frmspcledu0120MTrainingAttendanceRegisterPrintData from './Frmspcledu0120MTrainingAttendanceRegisterPrintData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmspcledu0120MTrainingAttendanceRegisterPrint = () => {
    const hook = useFrmspcledu0120MTrainingAttendanceRegisterPrint();
    return (
        <PageContainer>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmspcledu0120MTrainingAttendanceRegisterPrintData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack></Stack>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '396px', height: '22px', display: 'flex', alignItems: 'center', ml: '4px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmspcledu0120MTrainingAttendanceRegisterPrintData.ds_ds_oSort || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
export default Frmspcledu0120MTrainingAttendanceRegisterPrint;
