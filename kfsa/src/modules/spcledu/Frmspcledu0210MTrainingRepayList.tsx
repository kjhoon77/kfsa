...
import { useFrmspcledu0210MTrainingRepayList } from './useFrmspcledu0210MTrainingRepayList';
import * as Frmspcledu0210MTrainingRepayListData from './Frmspcledu0210MTrainingRepayListData';
import { FrmCOM0100SWorkFormTitle } from '../COM/FrmCOM0100SWorkFormTitle';

export const Frmspcledu0210MTrainingRepayList = () => {
    const hook = useFrmspcledu0210MTrainingRepayList();
    const columns_gdTrainingJubsu = [
        { field: 'GTDEPTNM', headerName: '지부', width: 60 },
        { field: 'TOTCCOURSECD', headerName: '직능', width: 110 },
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '392px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmspcledu0210MTrainingRepayListData.ds_ds_oSort || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
...
export default Frmspcledu0210MTrainingRepayList;
