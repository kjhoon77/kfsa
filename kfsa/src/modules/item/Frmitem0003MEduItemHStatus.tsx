...
export const Frmitem0003MEduItemHStatus = () => {
    ...
    const columns_gdYearList = [
        ...
        { field: 'TOTUSE_CNT', headerName: '사용 계', width: 63 },
    ...
    const columns_gdJibuItemList = [
        ...
        { field: 'TOTRECEIPT_CNT', headerName: '수령 계', width: 70 },
        { field: 'RECEIPT_CNT', headerName: '수령', width: 70 },
        { field: 'TAJIBURECEIPT_CNT', headerName: '타지부 수령', width: 70 },
        { field: 'TAJIBUUSE_CNT', headerName: '타지부 지원', width: 70 },
        { field: 'TOTUSE_CNT', headerName: '사용 계', width: 70 },
    ...
    const columns_gdItemJibuList = [
        ...
        { field: 'GTDEPTNM', headerName: '지부', width: 70 },
        { field: 'TOTRECEIPT_CNT', headerName: '수령 계', width: 60 },
        ...
        { field: 'TOTUSE_CNT', headerName: '사용 계', width: 60 },
    ...
    return (
        <PageContainer>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">교재수불현황</Typography>
...
<Grid container spacing={2}>
                ...
                <Grid item xs={12} md={12}>
                    ...
                    <FormControl component="fieldset" sx={{ width: "max-content", whiteSpace: "nowrap" }}><RadioGroup row sx={{ flexWrap: "nowrap" }} >{ (Frmitem0003MEduItemHStatusData.ds_ds_oReportGubun || []).map(opt => <FormControlLabel key={opt.CD} value={opt.CD} control={<Radio />} label={opt.DATA} sx={{ whiteSpace: 'nowrap', flexShrink: 0, mr: 2 }} />) }</RadioGroup></FormControl>
                 </Stack></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "4px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '170px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                 ...
                 </Stack><Box sx={{ display: hook.isVisible_calSubulDateSearchFr ? undefined : 'none' }}><DoubleClickDatePicker sx={{ width: "102px", "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiInputBase-input": { padding: "4px 0px", fontSize: "12px", marginLeft: "-2px" } }} /></Box></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '116px', height: '22px', display: 'flex', alignItems: 'center', ml: '2px' }}>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "3px", py: 0.5, width: "100%" }}><Box sx={{ ml: "4px", minWidth: "792px" }}><Box sx={{ display: hook.isVisible_gdYearList ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '484px', width: '100%', height: 'auto', minHeight: '484px' }}><MultiDataGridWrapper rows={hook.ds_ioYearList} columns={columns_gdYearList} rowHeight={20} headerHeight={56} hideFooter={true} /></Paper></Box><Box sx={{ display: hook.isVisible_gdJibuItemList ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '484px', width: '100%', height: 'auto', minHeight: '484px' }}><MultiDataGridWrapper rows={hook.ds_ioJibuItemList} columns={columns_gdJibuItemList} rowHeight={20} headerHeight={56} hideFooter={true} /></Paper></Box><Box sx={{ display: hook.isVisible_gdItemJibuList ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '484px', width: '100%', height: 'auto', minHeight: '484px' }}><MultiDataGridWrapper rows={hook.ds_ioItemJibuList} columns={columns_gdItemJibuList} rowHeight={20} headerHeight={56} hideFooter={true} /></Paper></Box><Box sx={{ display: hook.isVisible_gdYmdList ? undefined : 'none' }}><Paper sx={{ width: '792px', height: '484px', width: '100%', height: 'auto', minHeight: '484px' }}><MultiDataGridWrapper rows={hook.ds_ioYmdList} columns={columns_gdYmdList} rowHeight={20} headerHeight={56} hideFooter={true} /></Paper></Box></Box></Stack>
