...
export const Frmitem0021MEduEquipmentHMgmt = () => {
    ...
    const columns_gdEduEquipmentH = [
        ...
        { field: 'col_9', headerName: '이미지', width: 56 },
    ...
    return (
        <PageContainer>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Typography variant="h5">교육기자재관리</Typography>
...
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '432px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
                 ...
                 </Stack><Box sx={{ display: hook.isVisible_btnLoadImage ? undefined : 'none' }}><Button variant="contained" color="success"  onClick={hook.btnLoadImage_OnClick} sx={{ width: '108px', height: '24px', whiteSpace: "nowrap" }}>이미지 확대보기</Button></Box></Stack>
<Stack direction="row" alignItems="center" spacing={0} sx={{ mt: "2px", py: 0.5, width: "100%" }}><Box sx={{ display: hook.isVisible_Static16 ? undefined : 'none' }}><Box sx={{ display: "flex", alignItems: "center", width: '70px', height: '22px' }}><Typography variant="body2">폐기 등</Typography></Box></Box><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '154px', height: '22px', display: 'flex', alignItems: 'center', ml: '4px' }}>
                    <Typography variant="body2" sx={{ minWidth: 60, bgcolor: '#f5f5f5', p: 0.3, borderRadius: 1 }}>구분</Typography>
                    <FormControl size="small" fullWidth sx={{ "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(0,0,0,0.4)" }, "& .MuiSelect-select": { padding: "4px 6px !important" } }}><Select value={hook.ds_ioEduEquipmentHUnit?.EQHDISCARDGUBUN || ''} displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmitem0021MEduEquipmentHMgmtData.ds_ds_oDiscardGubun || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                 </Stack><Stack direction="row" alignItems="center" spacing={0.5} sx={{ width: '183px', height: '22px', display: 'flex', alignItems: 'center', ml: '8px' }}>
