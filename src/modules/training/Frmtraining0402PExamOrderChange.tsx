...
import { useFrmtraining0402PExamOrderChange } from './useFrmtraining0402PExamOrderChange';
import * as Frmtraining0402PExamOrderChangeData from './Frmtraining0402PExamOrderChangeData';

export const Frmtraining0402PExamOrderChange = () => {
    const hook = useFrmtraining0402PExamOrderChange();
    const columns_gdExamOrder = [
        { field: 'EOMGGTMGNO', headerName: '지부', width: 54 },
        { field: 'EOYEAR', headerName: '년도', width: 38 },
        { field: 'EOTCCOURSECD', headerName: '직능', width: 95 },
        { field: 'EOEXAMORDER', headerName: '회차', width: 37 },
        { field: 'EOEXAMDATE', headerName: '시험일', width: 83 },
...
<Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} md={4}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{  }}>
                        <Typography variant="body2" sx={{ minWidth: 80, display: 'flex', alignItems: 'center', bgcolor: '#f5f5f5', p: 0.5, borderRadius: 1 }}>지부</Typography>
                        <FormControl size="small" sx={{ width: 150 }}><Select value={hook.ds_oExamOrder?.EOMGGTMGNO || ''} displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0402PExamOrderChangeData.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    </Stack>
                </Grid></Grid>
<Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} md={4}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{  }}>
                        <Typography variant="body2" sx={{ minWidth: 80, display: 'flex', alignItems: 'center', bgcolor: '#f5f5f5', p: 0.5, borderRadius: 1 }}>직능</Typography>
                        <FormControl size="small" sx={{ width: 150 }}><Select value={hook.ds_oExamOrder?.EOTCCOURSECD || ''} displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0402PExamOrderChangeData.ds_ds_oCourse || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    </Stack>
                </Grid></Grid>
<Grid container spacing={2} alignItems="center">
...
export default Frmtraining0402PExamOrderChange;
