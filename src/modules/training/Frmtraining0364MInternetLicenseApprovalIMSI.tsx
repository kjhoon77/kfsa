...
<Grid container spacing={2} alignItems="center">
<Grid item xs={12} md={2}><Box></Box></Grid><Grid item xs={12} md={2}><Box></Box></Grid><Grid item xs={12} md={2}><Box></Box></Grid><Grid item xs={12} md={3}><Box>◆ 저장변경은 다중으로 처리할 수 없습니다.</Box></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnImsiSave_OnClick}>저장변경</Button></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnCancel_OnClick}>접수취소</Button></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnJubsu_OnClick}>접수시작</Button></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnMod_OnClick}>보완</Button></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnApprove_OnClick}>승인</Button></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnReject_OnClick}>반려</Button></Grid></Grid>
<Grid container spacing={2} alignItems="center">
<Grid item xs={12} md={2}><Box>개인정보</Box></Grid>
                ...
                ) }<Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnImageZoom_OnClick}>이미지확대</Button></Grid><Grid item xs={12} md={2}><Box>최근사진</Box></Grid></Grid>
<Grid container spacing={2} alignItems="center">
                ...
                </Grid>{ hook.isVisible_edAJTCCOURSECD && (
                <Grid item xs={12} md={2}>
                    <Paper sx={{ p: 2, height: '100%' }}>
...
<Grid item xs={12} md={2}><Box>신청정보</Box></Grid>
                ...
                <Grid item xs={12} md={4}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{  }}>
                        ...
                        <FormControl size="small" sx={{ width: 150 }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0364MInternetLicenseApprovalIMSIData.ds_ds_Issuegubun || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    </Stack>
                </Grid>{ hook.isVisible_btnSaveXml && (
                <Grid item xs={12} md={2}>
                    <Paper sx={{ p: 2, height: '100%' }}>
...
<Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} md={5}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{  }}>
                        ...
                        <FormControl size="small" sx={{ width: 150 }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmtraining0364MInternetLicenseApprovalIMSIData.ds_ds_oAjgubun || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    </Stack>
                </Grid>{ hook.isVisible_Button0 && (
                <Grid item xs={12} md={2}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: "bold" }}>sms</Typography>
                        <Grid container spacing={2}>
