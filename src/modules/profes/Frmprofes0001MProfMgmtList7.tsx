...
<Grid item xs={12} md={2}><Box>응답인원(응답률)</Box></Grid>
                ...
                <Grid item xs={12} md={4}>
                    <Stack direction="row" alignItems="center" spacing={1}>
                        ...
                        <FormControl size="small" fullWidth><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmprofes0001MProfMgmtList7Data.ds_ds_oJibu || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    </Stack>
                </Grid></Grid>
<Grid container spacing={2} alignItems="center">
<Grid item xs={12}><Box sx={{ width: "100%" }}><Box sx={{ borderBottom: 1, borderColor: "divider" }}><Tabs value={tabValue_TAB_Profes} onChange={(e, v) => setTabValue_TAB_Profes(v)} aria-label="TAB_Profes"><Tab label="강습교육만족도(과정별)" /><Tab label="실무교육만족도(과정별)" /><Tab label="강습교육만족도(항목별)" /><Tab label="실무교육만족도(항목별)" /><Tab label="강습교육만족도(지부별)" /><Tab label="실무교육만족도(지부별)" /></Tabs></Box><CustomTabPanel value={tabValue_TAB_Profes} index={0}></CustomTabPanel><CustomTabPanel value={tabValue_TAB_Profes} index={1}></CustomTabPanel><CustomTabPanel value={tabValue_TAB_Profes} index={2}></CustomTabPanel><CustomTabPanel value={tabValue_TAB_Profes} index={3}></CustomTabPanel><CustomTabPanel value={tabValue_TAB_Profes} index={4}></CustomTabPanel><CustomTabPanel value={tabValue_TAB_Profes} index={5}></CustomTabPanel></Box></Grid></Grid>
