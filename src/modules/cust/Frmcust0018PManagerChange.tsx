...
기존 정보가 변경되오니 신중히 변경하시기 바랍니다.</Box></Grid>{ hook.isVisible_lbText02 && (
                <Grid item xs={12} md={5}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: "bold" }}>선임자를 잘못 선임한 경우 선임대상자를 변경합니다.
선임자가 타인으로 변경되오니 신중히 변경하시기 바랍니다.</Typography>
                        <Grid container spacing={2}>
...
<Grid container spacing={2} alignItems="center">
                ...
                </Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnSearch_OnClick}>선임현황조회</Button></Grid></Grid>
<Grid container spacing={2} alignItems="center">
                ...
                </Grid><Grid item xs={12} md={2}><Box>   주민번호</Box></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnKeyinJumin_OnClick}>KEY
IN</Button></Grid><Grid item xs={12} md={2}><Box></Box></Grid>{ hook.isVisible_lbResidentNoError && (
                ...
                ) }<Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnAuthResidentNo_OnClick}>실명인증</Button></Grid></Grid>
<Grid container spacing={2} alignItems="center">
                ...
                </Grid><Grid item xs={12} md={2}><Box>휴대폰</Box></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnKeyinHP_OnClick}>KEY
IN</Button></Grid><Grid item xs={12} md={2}><Box></Box></Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnLicenseList_OnClick}>자격·수료정보조회</Button></Grid></Grid>
<Grid container spacing={2} alignItems="center">
...
<Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} md={4}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{  }}>
                        ...
                        <FormControl size="small" sx={{ width: 150 }}><Select  displayEmpty><MenuItem value=""><em>선택</em></MenuItem>{ (Frmcust0018PManagerChangeData.ds_ds_MemberGubun || []).map(opt => <MenuItem key={opt.CD} value={opt.CD}>{opt.DATA}</MenuItem>) }</Select></FormControl>
                    </Stack>
...
<Grid container spacing={2} alignItems="center">
                ...
                </Grid><Grid item xs={12} md={2}><Button variant="contained" onClick={hook.btnSaveManager_OnClick}>해임처리</Button></Grid><Grid item xs={12} md={2}><Tooltip title="BTN_MULTI_SORT"><Button variant="outlined" size="small" color="primary" onClick={hook.btnMutilSort_OnClick} sx={{ minWidth: 30, p: 0.5 }}><Sort fontSize="small" /></Button></Tooltip></Grid></Grid>
<Grid container spacing={2} alignItems="center">
...
<Grid container spacing={2} alignItems="center">
{ hook.isVisible_btnMemberSave && (
                <Grid item xs={12} md={2}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                        <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: "bold" }}>회원상태저장</Typography>
                        <Grid container spacing={2}>
